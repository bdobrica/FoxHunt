...
abstract class FH_Model {
	 ...
	 * similar to groups.
	 * @var array (list of FH_Model descendants to be linked to)
	 */
	protected static $L = [];
	/**
	 * Additional table to handle instances of this object, relative to object types listed in
	 * this array.
	 * @var array (list of FH_Model descendants that provide a unique pair {instance_key, instance_value})
	 */
	protected static $I = [];
	/**
	...
	public function set ($key = null, $value = null) {
		...
		if (is_array ($key)) {
			...
			if (!in_array($slug, static::$K)) throw new FH_Exception ();
			$this->data[$slug] = $value;

			if ($this->ID) {
				if (is_object ($value) && ($value instanceof FH_Model))
					$value = $value->get();
	...
	public function save () {
		...
		if (!empty(static::$K)) {
			...
			if (!defined ('FH_DEBUG')) {
				$wpdb->query ($sql);
				if (!($this->ID = $wpdb->insert_id)) throw new FH_Exception ();
				}
			else {
				$this->ID = rand ();
				FH_Model::debug ($sql);
